...
def intrinsic_value(dataset, attribute):
    '''Computes the intrinsic value of a dataset
    according to an attribute.

        IV(S, A) = - sum{k in values(A)}(size(S_k) / size(S) * log2(size(S_k) / size(S)))

    where:
    S: is the dataset;
    A: denotes an attribute;
    S_k: subset of S containing all the sample in S having attribute A
         assuming value k.
    '''
    weighted = 0.0
    for v in attribute.values:
        subset = select(dataset, attribute, v)
        weighted += log2(1. * len(subset) / len(dataset)) * len(subset)
    return - weighted/len(dataset)


def info_gain_ratio(dataset, attribute):
    '''Computes the information gain ratio of a dataset
    according to an attribute.

        IGR(S, A) = IG(S, A) / IV(S, A)
    '''
    return averageGain(dataset, attribute) / intrinsic_value(dataset, attribute)
